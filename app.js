...
function currentCheckoutTotal(){
  ...
  return Number(checkout.basePrice||0)+Number(checkout.cabinExtra||0)+(extras.bike?8:0)+(extras.upgrade?24:0)+(extras.seatReservation?6:0);
}
...
function requestCheckoutCancel(){
  ...
  showModal(t('cancelPurchase'),`<article class="cancel-confirm"><div class="cancel-symbol">×</div><h3>${esc(t('cancelQuestion'))}</h3><p>${esc(t('cancelWarning'))}</p><div class="cancel-actions"><button id="keepCheckout" class="primary full" type="button">${esc(t('keepBuying'))}</button><button id="discardCheckout" class="danger-button full" type="button">${esc(t('discardPurchase'))}</button></div></article>`,{closable:false});
  document.getElementById('keepCheckout').addEventListener('click',()=>{if(checkout?.id===session)resumeCheckout()});
  document.getElementById('discardCheckout').addEventListener('click',()=>{paymentRun++;checkout=null;closeModal();toast(t('purchaseCancelled'))});
}
...
function startJourneyCheckout(journey,fare){
  checkout={id:uid('CHK'),kind:'journey',journey,fare,step:journey.type==='RE'?2:1,seat:null,cabinExtra:0,extras:{bike:false,upgrade:false,seatReservation:false},traveler:{name:'David J. Martínez',document:'',documentsConfirmed:false},basePrice:Math.round(journey.price*fare.factor),paymentFields:{}};
  checkout.price=currentCheckoutTotal();
  renderCheckout();
}
function renderCheckout(){
  ...
  showModal(titles[Math.max(0,checkout.step-1)]||t('payment'),`<div class="checkout-progress" aria-label="${esc(t('reviewOrder'))}">${[1,2,3,4].map(n=>`<span class="${n<=checkout.step?'active':''}">${n}</span>`).join('')}</div><div id="checkoutBody"></div>`,{closable:true,onRequestClose:requestCheckoutCancel,closeLabel:t('cancel')});
...
function renderSeatStep(){
  ...
  if(night){
    ...
    body.querySelectorAll('[data-cabin]').forEach(b=>b.addEventListener('click',()=>{body.querySelectorAll('[data-cabin]').forEach(x=>x.classList.remove('selected'));b.classList.add('selected');checkout.cabinExtra=Number(b.dataset.price);checkout.seat=b.dataset.cabin;checkout.price=currentCheckoutTotal()}));
  }else{
    body.innerHTML=`<div class="seat-legend"><span><i class="available"></i>${esc(t('seat'))}</span><span><i class="occupied"></i>${esc(t('occupied'))}</span><span><i class="selected"></i>${esc(t('chosen'))}</span></div><div class="coach"><div class="coach-label">Wagen 5 · ${esc(t('quiet'))}</div><div class="seat-grid">${makeSeats(checkout.journey.id,checkout.seat)}</div></div><p id="seatStatus" class="muted">${checkout.seat?`${esc(t('seat'))}: ${esc(checkout.seat)}`:esc(t('seatSelection'))}</p>${checkoutButtons({back:false,nextId:'seatNext',disabled:!checkout.seat})}`;
  ...
  bindCheckoutShell();
  document.getElementById('seatNext').addEventListener('click',()=>{checkout.step=2;renderCheckout()});
}
...
function renderTravelerStep(){
  ...
  b.innerHTML=`<div class="field"><label for="travelerName">${esc(t('traveler'))}</label><input id="travelerName" value="${esc(checkout.traveler.name)}"></div>${checkout.journey.international?`<article class="card federal-card standalone"><span>BPOL</span><div><h3>${esc(t('federalPolice'))}</h3><p>${esc(t('borderText'))}</p></div></article><div class="field"><label for="documentNumber">${esc(t('passportDemo'))}</label><input id="documentNumber" placeholder="GZ1234567" maxlength="12" value="${esc(checkout.traveler.document||'')}"></div><label class="check-row"><input id="documentsConfirmed" type="checkbox" ${checkout.traveler.documentsConfirmed?'checked':''}><span>${esc(t('documentsConfirm'))}</span></label>`:''}${checkoutButtons({back:checkout.journey.type!=='RE',nextId:'travNext'})}`;
  bindCheckoutShell(()=>{checkout.step=1;renderCheckout()});
  document.getElementById('travNext').addEventListener('click',()=>{const name=document.getElementById('travelerName').value.trim();if(name.length<3){toast(t('nameRequired'));return}const confirmed=document.getElementById('documentsConfirmed')?.checked||false;if(checkout.journey.international&&!confirmed){toast(t('passportRequired'));return}checkout.traveler={name,document:document.getElementById('documentNumber')?.value||'',documentsConfirmed:confirmed};checkout.step=3;renderCheckout()});
}
function renderExtrasStep(){
  const b=document.getElementById('checkoutBody');
  const e=checkout.extras||{};
  b.innerHTML=`<div class="extras-list"><label class="extra-row"><input id="seatReservation" type="checkbox" ${checkout.seat?'checked disabled':e.seatReservation?'checked':''}><span><strong>${esc(t('seatReservation'))}</strong><small>${checkout.seat?esc(checkout.seat):'+ 6 GM'}</small></span></label><label class="extra-row"><input id="bikeExtra" type="checkbox" ${e.bike?'checked':''}><span><strong>${esc(t('bike'))}</strong><small>+ 8 GM</small></span></label><label class="extra-row"><input id="classUpgrade" type="checkbox" ${e.upgrade?'checked':''}><span><strong>${esc(t('firstClass'))}</strong><small>+ 24 GM</small></span></label></div><div class="summary-card"><span>${esc(t('subtotal'))}</span><strong id="extraTotal">${esc(currency(currentCheckoutTotal()))}</strong></div>${checkoutButtons({back:true,nextId:'extraNext'})}`;
  const update=()=>{checkout.extras={bike:document.getElementById('bikeExtra').checked,upgrade:document.getElementById('classUpgrade').checked,seatReservation:!checkout.seat&&document.getElementById('seatReservation').checked};checkout.price=currentCheckoutTotal();document.getElementById('extraTotal').textContent=currency(checkout.price)};
  ['bikeExtra','classUpgrade','seatReservation'].forEach(id=>document.getElementById(id).addEventListener('change',update));
  bindCheckoutShell(()=>{checkout.step=2;renderCheckout()});
  document.getElementById('extraNext').addEventListener('click',()=>{update();checkout.step=4;renderCheckout()});
}
function renderPaymentStep(){
  ...
  bindCheckoutShell(()=>{capturePaymentFields();checkout.step=3;renderCheckout()});
...
function renderDirectPaymentCheckout(){
  ...
  bindCheckoutShell(()=>{capturePaymentFields();const kind=checkout.kind;checkout=null;closeModal();if(kind==='subscription')openSubscriptions();else navigate('city')});
...
function finalizeSubscription(result){
  ...
  store.set(s=>({...s,subscriptions:[sub,...s.subscriptions.map(x=>({...x,status:'replaced'}))],orders:[{id:result.authorization,type:'subscription',description:product.name,amount:product.price,date:new Date().toISOString()},...s.orders]}));paymentRun++;checkout=null;closeModal();toast(t('approved'));navigate('tickets');
}
function finalizeLocalTicket(result){
  ...
  store.set(s=>({...s,tickets:[ticket,...s.tickets],orders:[{id:result.authorization,type:'urban',description:`${ticket.name} · ${network}`,amount:ticket.price,date:new Date().toISOString()},...s.orders]}));paymentRun++;checkout=null;closeModal();toast(t('ticketBooked'));navigate('tickets');
}
...
function finalizeJourney(payment){
  const j=checkout.journey;checkout.price=currentCheckoutTotal();
  const ticket={...j,id:uid('GBT'),fare:checkout.fare.name,price:checkout.price,coach:j.night?'NJ':String(2+seed(j.id)%7),seat:checkout.seat||'Ohne Reservierung',passenger:checkout.traveler.name,document:checkout.traveler.document,extras:checkout.extras,payment:{brand:payment.card.brand,last4:payment.last4,authorization:payment.authorization},bookedAt:new Date().toISOString()};
  store.set(s=>({...s,tickets:[ticket,...s.tickets],orders:[{id:payment.authorization,type:'ticket',description:`${j.train} ${j.from} – ${j.to}`,amount:checkout.price,date:new Date().toISOString()},...s.orders],activeTicketId:ticket.id}));paymentRun++;checkout=null;closeModal();toast(t('ticketBooked'));navigate('tickets');
}
function renderCity(){
...
function openUrbanLine(network,lineId){const l=network.lines.find(x=>x.id===lineId);showModal(`${l.id} · ${network.name}`,`<div class="line-detail"><div class="line-header"><span class="line-pill big" style="--line:${l.color}">${esc(l.id)}</span><div><strong>${esc(l.stops[0])} ↔ ${esc(l.stops.at(-1))}</strong><small>${esc(t('frequency'))}: ${l.minutes} min</small></div></div><div class="urban-timeline">${l.stops.map((s,i)=>`<div><span></span><strong>${esc(s)}</strong><small>${addTime(new Date().toTimeString().slice(0,5),i*3)}</small></div>`).join('')}</div><button class="secondary full modal-bottom-cancel" id="lineClose" type="button">${esc(t('close'))}</button></div>`,{closable:true});document.getElementById('lineClose').addEventListener('click',()=>closeModal())}
function startLocalTicketCheckout(type,network,routeData=null){checkout={id:uid('CHK'),kind:'local',product:{type,name:type==='day'?t('dayTicket'):t('singleTicket'),price:type==='day'?14:4,network:network.name,routeData},basePrice:type==='day'?14:4,price:type==='day'?14:4,step:4,paymentFields:{},origin:'city'};renderDirectPaymentCheckout()}
...
function startSubscriptionCheckout(product){checkout={id:uid('CHK'),kind:'subscription',product,basePrice:product.price,price:product.price,step:4,paymentFields:{},origin:'subscriptions'};renderDirectPaymentCheckout()}
...
function renderProfile(){const s=state(),sub=activeSubscription();main.innerHTML=`<section class="page-header"><p class="eyebrow">GB Card</p><h1>${esc(t('profile'))}</h1></section><article class="card profile-card"><div class="profile-head"><span>DJ</span><div><h2>David J. Martínez</h2><p>GB Card Gold</p></div></div><div class="gold-card"><small>GB CARD GOLD</small><strong>2048 8361 0917</strong></div><div class="stats"><div><strong>${124+s.tickets.length}</strong><small>${esc(t('journeys'))}</small></div><div><strong>${(36000+s.tickets.length*240).toLocaleString(locale())}</strong><small>km</small></div><div><strong>${(8420+s.orders.length*230).toLocaleString(locale())}</strong><small>${esc(t('points'))}</small></div></div></article>${sub?`<section class="section"><div class="section-title"><h2>${esc(t('activeSubscription'))}</h2></div>${subscriptionCard(sub)}<button class="danger-button full" data-cancel-sub="${sub.id}">${esc(t('cancelSubscription'))}</button></section>`:''}<section class="section"><div class="section-title"><h2>${esc(t('paymentMethods'))}</h2></div><article class="card setting-row"><span>▰</span><div><strong>${s.savedPayment?`${esc(s.savedPayment.brand)} •••• ${esc(s.savedPayment.last4)}`:esc(t('testCardMissing'))}</strong><small>${esc(t('demoOnly'))}</small></div></article></section><section class="section"><div class="section-title"><h2>${esc(t('purchaseHistory'))}</h2></div><div class="order-list">${s.orders.slice(0,5).map(o=>`<article class="card order-row"><div><strong>${esc(o.description)}</strong><small>${new Date(o.date).toLocaleDateString(locale())}</small></div><b>${esc(currency(o.amount))}</b></article>`).join('')||'<p class="muted">—</p>'}</div></section><section class="section"><div class="section-title"><h2>${esc(t('settings'))}</h2></div><article class="card settings"><button id="languageSetting"><span class="setting-icon">文</span><div><strong>${esc(t('language'))}</strong><small>${esc(languageName(s.language))}</small></div><b>›</b></button><button id="themeSetting"><span class="setting-icon">◐</span><div><strong>${esc(t('appearance'))}</strong><small>${esc(themeName(s.theme))}</small></div><b>›</b></button><button id="installSetting"><span class="setting-icon">⇩</span><div><strong>${esc(t('install'))}</strong><small>PWA · ${APP_VERSION}</small></div><b>›</b></button></article></section><p class="build-note">v${APP_VERSION} · ${APP_BUILD}</p>`;
...
function register(){
  ...
  window.addEventListener('hashchange',()=>render(routeFromHash()));window.addEventListener('online',updateOnline);window.addEventListener('offline',updateOnline);
  ...
  if('serviceWorker'in navigator&&location.protocol.startsWith('http')){
    navigator.serviceWorker.register('./sw.js').then(reg=>{
      ...
      reg.addEventListener('updatefound',()=>{const worker=reg.installing;worker?.addEventListener('statechange',()=>{if(worker.state==='installed'&&navigator.serviceWorker.controller)showUpdateBanner(reg)})});
    }).catch(console.warn);
