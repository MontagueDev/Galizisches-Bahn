...
const t = (key, vars={}) => {
  let text = EXTRA_COPY[state().language]?.[key] ?? COPY[state().language]?.[key] ?? EXTRA_COPY.de[key] ?? COPY.de[key] ?? key;
...
function renderHome(){
  const search=state().search,trip=activeTicket(),sub=activeSubscription();
  main.innerHTML=`
...
function renderCity(){
  const urban=state().urban,network=getUrbanNetwork(urban.cityId),view=state().ui.cityView||'planner',mode=state().ui.cityMode||'all';
...
function openStation(station){showModal(t('station'),`<article><p class="eyebrow">${esc(station.id)} · ${esc(station.country)}</p><h2>${esc(station.name)}</h2><p class="muted">${esc(station.oldName)} · ${station.platforms} ${esc(t('platform'))}</p>${station.id==='SJR'?federalNotice():station.country==='MX'?`<article class="official-advisory-inline"><strong>Auswärtiges Amt</strong><p>${esc(t('travelWarningBody'))}</p></article>`:''}<h3>${esc(t('services'))}</h3><div class="chips">${station.amenities.map(a=>`<i>${esc(a)}</i>`).join('')}</div><div class="button-row"><button class="secondary" id="stationClose">${esc(t('close'))}</button><button class="primary" id="stationSearch">${esc(t('search'))}</button></div></article>`,{closable:true});document.getElementById('stationClose').addEventListener('click',()=>closeModal());document.getElementById('stationSearch').addEventListener('click',()=>{store.set(s=>({...s,search:{...s.search,fromId:station.id,toId:station.id==='MEX'?'GUA':state().search.toId}}));closeModal();navigate('travel')})}
