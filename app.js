...
function renderJourneyResults(){
  ...
  list.innerHTML=journeys.map((j,index)=>`<button class="journey-card v08-journey-card ${j.international?'international-journey':''}" data-journey="${esc(j.id)}" type="button">
    <div class="journey-card-head"><div><span class="service-badge ${esc(j.type)}">${esc(j.train)}</span>${index===0?`<span class="recommend-chip">${esc(t('recommended'))}</span>`:''}</div><span class="status ${j.delay?'warn':'ok'}">${j.delay?`+${j.delay} min`:esc(t('onTime'))}</span></div>
    <div class="journey-time-grid"><div><strong>${esc(j.departure)}</strong><small>${esc(j.from)}</small></div><span class="journey-line"><i></i><em>${esc(duration(j.duration))}</em></span><div><strong>${esc(j.arrival)}</strong><small>${esc(j.to)}</small></div></div>
    <div class="journey-meta-row"><span>${j.changes?`${j.changes} ${esc(t('changes'))}`:esc(t('direct'))}</span><span class="occupancy-text ${esc(j.occupancy)}">${esc(occupancyLabel(j.occupancy))}</span>${j.international?`<span>SJR · ${j.borderMinutes} min</span>`:''}</div>
    ${j.international?`<div class="border-inline-flow"><span>${esc(j.fromId==='MEX'?t('mexicoFlowInbound'):t('mexicoFlowOutbound'))}</span></div>`:''}
    <div class="journey-price-row"><span>${j.duration===fast?esc(t('fastest')):''}</span><strong>${j.price===best?`<small>${esc(t('bestPrice'))}</small>`:''}${esc(currency(j.price))}<b>›</b></strong></div>
  </button>`).join('');
...
function createTicketPdf(ticket){
  const policy=ticketRefundPolicy(ticket),lines=['GALIZISCHE BAHN',ticket.status==='cancelled'?t('cancellationReceipt').toUpperCase():t('digitalTicketPdf').toUpperCase(),`${t('ticketLabel')}: ${ticket.id}`,ticket.kind==='urban'?`${ticket.name} - ${ticket.network}`:`${ticket.from} -> ${ticket.to}`,ticket.kind==='urban'?`${t('validUntil')}: ${ticket.validUntil}`:`${t('date')}: ${ticket.date}  ${ticket.departure}-${ticket.arrival}`,`${t('price')}: ${ticket.price} GM`,`${t('ticketStatus')}: ${ticket.status==='cancelled'?t('cancelledTicket'):t('validTicket')}`];
  if(ticket.status==='cancelled')lines.push(`${t('refund')}: ${policy.amount} GM`,`${t('cancelledAt')}: ${ticket.cancelledAt}`);
